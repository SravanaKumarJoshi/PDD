...
def load_data_from_mysql_or_fallback() -> pd.DataFrame:
    """Load data strictly from MySQL database."""
    ...
    try:
        ...
        if df.empty:
            raise RuntimeError(f"MySQL query returned 0 records from table '{MATERIAL_TABLE_NAME}'.")
        print(f"[TrainPipeline] Loaded {len(df)} rows from MySQL table '{MATERIAL_TABLE_NAME}'.")
        return df
    except Exception as e:
        print(f"[TrainPipeline] ERROR: Could not connect to MySQL or load table '{MATERIAL_TABLE_NAME}': {e}")
        raise RuntimeError(f"Database error: Failed to query MySQL data source ({e})") from e
