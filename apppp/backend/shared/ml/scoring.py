...
def calculate_material_score_details(
    ...
    if target_tensile is not None and float(target_tensile) > 0:
        ...
        if act_tensile is not None and float(act_tensile) >= 0:
            match_pct = min(float(act_tensile) / float(target_tensile), 1.0) * 100.0
            mech_props["tensile_strength"] = {
                ...
                "is_used": True,
                "is_missing": False,
                "penalty_applied": None if match_pct >= 100.0 else f"Ratio match ({round(match_pct, 1)}%)",
            }
        else:
            mech_props["tensile_strength"] = {
                "requested": f">= {target_tensile} MPa",
                ...
                "is_used": False,
                "is_missing": True,
                "penalty_applied": "Missing property value in database",
            }
    ...
    if target_modulus is not None and float(target_modulus) > 0:
        ...
        if act_modulus is not None and float(act_modulus) >= 0:
            match_pct = min(float(act_modulus) / float(target_modulus), 1.0) * 100.0
    ...
    if mech_props:
        ...
        valid_matches = [p["match_pct"] for p in mech_props.values() if not p["is_missing"]]
        cat_score = sum(valid_matches) / len(valid_matches) if valid_matches else 0.0
    ...
    if target_wvtr is not None and float(target_wvtr) > 0:
        ...
        if act_wvtr is not None and float(act_wvtr) >= 0:
            if float(act_wvtr) <= float(target_wvtr):
                match_pct = 100.0
                penalty = None
            else:
                ratio = float(target_wvtr) / float(act_wvtr)
                match_pct = max(0.0, ratio * 100.0)
                penalty = f"Exceeds max WVTR limit ({float(act_wvtr)} > {target_wvtr})"
            barrier_props["wvtr"] = {
    ...
    if barrier_props:
        ...
        valid_matches = [p["match_pct"] for p in barrier_props.values() if not p["is_missing"]]
        cat_score = sum(valid_matches) / len(valid_matches) if valid_matches else 0.0
    ...
    if min_biocompat is not None:
        ...
        if act_biocompat is not None:
            match_pct = min(float(act_biocompat) / float(max(float(min_biocompat), 1.0)), 1.0) * 100.0
            bio_props["biocompatibility"] = {
                ...
                "is_used": True,
                "is_missing": False,
                "penalty_applied": None if match_pct >= 100.0 else f"Below minimum biocompatibility threshold ({act_biocompat} < {min_biocompat})",
            }
        else:
            bio_props["biocompatibility"] = {
                "requested": f">= {min_biocompat} / 10",
    ...
    if bio_props:
        ...
        valid_matches = [p["match_pct"] for p in bio_props.values() if not p["is_missing"]]
        cat_score = sum(valid_matches) / len(valid_matches) if valid_matches else 0.0
    ...
    if target_biodeg is not None and float(target_biodeg) > 0:
        ...
        if act_biodeg is not None:
            diff = abs(float(act_biodeg) - float(target_biodeg))
            match_pct = max(0.0, 1.0 - (diff / 365.0)) * 100.0
            deg_props["biodegradation_days"] = {
                ...
                "is_used": True,
                "is_missing": False,
                "penalty_applied": None if match_pct >= 90.0 else f"Deviation from target ({act_biodeg} vs {target_biodeg} days)",
            }
        else:
            deg_props["biodegradation_days"] = {
                "requested": f"~ {target_biodeg} days",
    ...
    if deg_props:
        ...
        valid_matches = [p["match_pct"] for p in deg_props.values() if not p["is_missing"]]
        cat_score = sum(valid_matches) / len(valid_matches) if valid_matches else 0.0
    ...
    final_rule_score = (weighted_scores_sum / active_weights_sum) if active_weights_sum > 0 else 75.0
    return round(float(final_rule_score), 2), category_breakdown
...
def rank_candidates(
    ...
    results_df["ml_probability"] = np.round(ml_probs_1d, 4)
    ...
    results_df["final_score"] = np.round(
        (results_df["rule_score"] * RULE_SCORE_WEIGHT) + (results_df["ml_score"] * ML_SCORE_WEIGHT),
        2
    )
